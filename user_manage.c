...
void write()
{
    ...
    int i;
    for(i=0;i<n;i++)
		fprintf(fp,"%s,%s,%s,%s,%s\n",s[i].usrn,encrypt(s[i].pswd),s[i].name,encrypt(s[i].sq),encrypt(s[i].sa));
    fclose(fp);
}
void openfile_user()
{
...
void change_user_details()
{
    ...
    scanf("%[^\n]s",choice);
 +  fflush(stdin);
    if(!(!strcmp(choice,"4")||!strcmpi(choice,"Go Back")))
    {
        if(!strcmp(choice,"1")||!strcmpi(choice,"change name")||strcmpi(choice,"name"))
        {
            TC_CLRSCR();
            TC_MOVE_CURSOR(0,0);
            int flag=0;
        ...
        else if(!strcmp(choice,"2")||!strcmpi(choice,"change Security Question and Answer"))
        {
            ...
            do
            {
                ...
                if((!strcmpi("Yes",ch))||(!strcmpi("1",ch)))
                {
                    ...
                    printf("\n Answer : ");
                    gets(s[curr_user].sa);
                    printf("\n Your Verification Details have been updated.\n");
                    Sleep(1500);
                    change_user_details();
                }
                else if((!strcmpi("No",ch))||(!strcmpi("2",ch)))
        ...
        else if(!strcmp(choice,"3")||!strcmpi(choice,"Reset scores")||strcmpi(choice,"reset"))
        {
            TC_CLRSCR();
            TC_MOVE_CURSOR(0,0);
...
int user_settings()
{
    ...
    printf("\n1. View User info\n2. Change Password\n3. Change Other User info \n4. Delete User \n5. Go Back\n\n");
    char choice[20];
    scanf("%[^\n]s",choice);
    fflush(stdin);
    if(!strcmp(choice,"1")||!strcmpi(choice,"View User Details")||!strcmpi(choice,"View"))
    {
        TC_CLRSCR();
        TC_MOVE_CURSOR(0,0);
        printf("Your Details :\n\n");
    ...
    else if(!strcmp(choice,"5")||!strcmpi(choice,"back")||!strcmpi(choice,"go back")||!strcmpi(choice,"go")||!strcmpi(choice,"goback"))
    {
        return 0;
    }
    else if(!strcmp(choice,"4")||!strcmpi(choice,"delete user")||!strcmpi(choice,"delete"))
    {
        if(!delete_user())
            return user_settings();
        else 
...
int delete_user()
{
    ...
    {
        int n=printf("Incorrect password. Try again");
        count++;
        if(count<2)
        {
            Sleep(1500);
